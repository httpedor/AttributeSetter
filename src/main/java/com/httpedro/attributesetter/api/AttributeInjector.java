...
public class AttributeInjector {
    ...
    // Deterministic UUID from a descriptive key, so repeated calls yield equal (deduped) modifiers.
    private static UUID idFor(String key) {
        return UUID.nameUUIDFromBytes((Attributesetter.MODID + ":" + key).getBytes());
    }
    ...
    public static Collection<AttributeModifier> getInjectionsFor(ResourceLocation attrId, AttributeModifier.Operation op, LivingEntity entity)
    {
        ...
        for (var entry : getEntriesForAttributeOp(attrId, op))
        {
            ...
            var otherAttr = ForgeRegistries.ATTRIBUTES.getValue(otherAttrId);
            if (otherAttr == null)
                continue;
            ...
            if (conversion != null)
            {
                for (var mod : ((EntityAttributeInstance)otherAttrInstance).getModifiersOrEmptyExposed(entry.sourceOp, false)) {
                    var converted = conversion.apply((float)mod.getAmount());
                    var name = "attr_copy_" + otherAttrId.getPath() + "/" + mod.getId() + "/" + i;
                    modifiers.add(new AttributeModifier(idFor(name), name, converted, op));
                }
            }
            i++;
        }
        ...
        for (var entry : getBaseEntriesForAttributeOp(attrId, op))
        {
            ...
            if (conversion != null)
            {
                ...
                var name = "attr_copy_base_" + otherAttrId.getPath() + "/" + i;
                modifiers.add(new AttributeModifier(idFor(name), name, converted, op));
            }
            i++;
        }
        ...
        for (var entry : getFinalEntriesForAttributeOp(attrId, op))
        {
            ...
            if (conversion != null)
            {
                ...
                var name = "attr_copy_final_" + otherAttrId.getPath() + "/" + i;
                modifiers.add(new AttributeModifier(idFor(name), name, converted, op));
            }
            i++;
        }
    ...
    public static Collection<Attribute> getAttributesDependentOn(ResourceLocation attrId)
    {
        ...
        if (dependents != null) {
            ...
            {
                var attr = ForgeRegistries.ATTRIBUTES.getValue(dependent);
                if (attr != null)
                    result.add(attr);
            }
        }
